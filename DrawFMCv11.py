...
#post: devuelve el otro punto extremo de la hipotenusa
	...
	if cuad==1:
		...
		ang=180-ang#complementario
		a=math.cos(math.radians(ang))* radio
		b=math.sin(math.radians(ang))* radio
		xd=xCenter - a
		yd=xCenter - b
	elif cuad==3:
		ang=ang-180
...
def main():
	...
	print ("este es el centroX: ", centroX)
	print ("este es el centroY: ", centroY)
	cuadrante=dameCuadrante(anguloIni)
	print ("el anguloini es : ",anguloIni)
	print ("esta en el cuadrante: ",cuadrante)

	cuadrante=dameCuadrante(anguloFin)
	print ("el anguloFin es : ",anguloFin)
	print ("esta en el cuadrante: ",cuadrante)
	px0,py0 = damePtoArcoReloj(centroX,centroY,anguloIni,base)
	print ("este es la X del punto inicial lado1 del troncoCono:", px0)
	print ("este es la Y del punto inicial lado1 del troncoCono:", py0)
	# pxd,pyd = damePtoArcoReloj(centroX,centroY,anguloIni,altura)
	# print ("este es la X del punto destino lado1 del troncoCono:", pxd)
	# print ("este es la Y del punto destino lado1 del troncoCono:", pyd)
	#drawLineColoured(drawing,px0, py0, pxd, pyd, colorLine)
	#print ("dibujado linea lado1 del troncoCono")
	drawCircleColoured(drawing,20,centroX,centroY,colorLine)
	print ("dibujado el circulo interno del calendario")
	#drawCircleColoured(drawing,40,centroX,centroY,colorLine)
	#print ("dibujado el circulo externo del calendario")
	drawLineColoured(drawing,centroX, centroY, px0, py0, colorLine2)
#	drawLineColoured(drawing,centroX, centroY, pxd, pyd, colorLine2)
	print ("dibujado linea de centro a base angulo adyacente")
	dibujaArcoRelojColoreado(drawing,centroX,centroY,base,anguloIni,anguloFin,colorLine2)
#	drawArcColoured(drawing,centroX,centroY,base,anguloIni,anguloFin,colorLine2)
	print ("dibujado el arco superior")
	muestraTriangulo(drawing,centroX,centroY,anguloIni,base,colorLine3)
	#oldX,oldY=shiftRight(p0x, p0y, 15.0)
	#oldX,oldY=drawPieceNumberWhite(drawing,oldX,oldY,1,6.0,3)
	#oldX,oldY=shiftRight(oldX, oldY, 15.0)
	#oldX,oldY=drawPoolPiecesNumbered(drawing,p0x,p0y,radArcMin,20,4,space)
	#oldX,oldY=shiftNextLine(oldX,oldY,radArcMin,20,4,1,radArcMin * 5,p0x)
	#oldX,oldY=shiftNextLinePiecesNumered(oldX,oldY,radArcMin,20,4,p0x)
	#oldX,oldY=drawPoolPiecesNumeredWhite(drawing,p0x,p0y,radArcMin,40,8,space)
	#oldX,oldY=shiftNextLine(oldX,oldY,radArcMin,20,4,1,radArcMin * 11,p0x)
	#oldX,oldY=drawPoolPiecesCommandsWhite(drawing,oldX,oldY,radArcMin,10,2,space)
	#oldX,oldY=shiftNextLine(oldX,oldY,radArcMin,40,4,1,radArcMin * 11,p0x)
	#oldX,oldY=drawPieceTopLoopWhite(drawing,p0x,p0y,radArcMin)
	#oldX,oldY=shiftNextLine(oldX,oldY,radArcMin,40,4,3,radArcMin * 25,p0x)
	#oldX,oldY=drawArrowToLeft(drawing,oldX,oldY,radArcMin,colorText)
	#oldX,oldY=shiftRight(oldX,oldY, space)
	#oldX,oldY=drawArrowToRight(drawing,oldX,oldY,radArcMin,colorText)
	#oldX,oldY=shiftRight(oldX,oldY, space)
	#oldX,oldY=drawArrowToUp(drawing,oldX,oldY,radArcMin,colorText)
	#oldX,oldY=shiftNextLine(oldX,oldY,radArcMin,40,4,3,radArcMin * 25,p0x)
	#oldX,oldY=drawPieceTopForeverCustom(drawing,oldX,oldY,radArcMin,30.0,shiftTopLineText,sizeText,1)
	#oldX,oldY=shiftNextLine(oldX,oldY,radArcMin,40,4,3,radArcMin * 25,p0x)
	#oldX,oldY=drawPieceTopRepeatCustom(drawing,oldX,oldY,radArcMin,30.0,shiftTopLineText,sizeText,1)
	#oldX,oldY=shiftNextLine(oldX,oldY,radArcMin,40,4,3,radArcMin * 25,p0x)
	#oldX,oldY=drawPieceBottomForeverCustom(drawing,oldX,oldY,radArcMin,30,2.5,radArcMin)
	#oldX,oldY=shiftNextLine(oldX,oldY,radArcMin,40,4,3,radArcMin * 25,p0x)
	#oldX,oldY=drawPieceBottomRepeatCustom(drawing,oldX,oldY,radArcMin,30,2.5,radArcMin)
##	oldX,oldY=shiftNextLine(p0x,p0y,radArcMin,40,4,3,radArcMin * 25,p0x)
##	oldX,oldY=drawPieceWait(drawing,oldX,oldY,radArcMin,1)
##	oldX,oldY=shiftNextLine(oldX,oldY,radArcMin,40,4,3,radArcMin * 25,p0x)
##	oldX,oldY=drawPieceMove(drawing,oldX,oldY,radArcMin,1)
##	oldX,oldY=shiftNextLine(oldX,oldY,radArcMin,40,4,3,radArcMin * 25,p0x)
##	oldX,oldY=drawPieceTurnLeft(drawing,oldX,oldY,radArcMin,1)
##	oldX,oldY=shiftNextLine(oldX,oldY,radArcMin,40,4,3,radArcMin * 25,p0x)
##	oldX,oldY=drawPieceTurnRight(drawing,oldX,oldY,radArcMin,1)
##	oldX,oldY=shiftNextLine(oldX,oldY,radArcMin,40,4,3,radArcMin * 25,p0x)
##	oldX,oldY==drawPieceTopRepeat(drawing,oldX,oldY,radArcMin,1)
##	oldX,oldY=shiftNextLine(oldX,oldY,radArcMin,40,4,3,radArcMin * 25,p0x)
##	oldX,oldY==drawPieceTopForever(drawing,oldX,oldY,radArcMin,1)
##	oldX,oldY=shiftNextLine(oldX,oldY,radArcMin,40,4,3,radArcMin * 25,p0x)
##	oldX,oldY==drawPieceBottomRepeat(drawing,oldX,oldY,radArcMin,1)
##	oldX,oldY=shiftNextLine(oldX,oldY,radArcMin,40,4,3,radArcMin * 25,p0x)
##	oldX,oldY==drawPieceBottomForever(drawing,oldX,oldY,radArcMin,1)
##	oldX,oldY=shiftNextLine(oldX,oldY,radArcMin,40,4,3,radArcMin * 25,p0x)
##	oldX,oldY=drawPieceWait(drawing,oldX,oldY,radArcMin,0)
##	oldX,oldY=shiftNextLine(oldX,oldY,radArcMin,40,4,3,radArcMin * 25,p0x)
##	oldX,oldY=drawPieceMove(drawing,oldX,oldY,radArcMin,0)
##	oldX,oldY=shiftNextLine(oldX,oldY,radArcMin,40,4,3,radArcMin * 25,p0x)
##	oldX,oldY=drawPieceTurnLeft(drawing,oldX,oldY,radArcMin,0)
##	oldX,oldY=shiftNextLine(oldX,oldY,radArcMin,40,4,3,radArcMin * 25,p0x)
##	oldX,oldY=drawPieceTurnRight(drawing,oldX,oldY,radArcMin,0)
##	oldX,oldY=shiftNextLine(oldX,oldY,radArcMin,40,4,3,radArcMin * 25,p0x)
##	oldX,oldY==drawPieceTopRepeat(drawing,oldX,oldY,radArcMin,0)
##	oldX,oldY=shiftNextLine(oldX,oldY,radArcMin,40,4,3,radArcMin * 25,p0x)
##	oldX,oldY==drawPieceTopForever(drawing,oldX,oldY,radArcMin,0)
##	oldX,oldY=shiftNextLine(oldX,oldY,radArcMin,40,4,3,radArcMin * 25,p0x)
##	oldX,oldY==drawPieceBottomRepeat(drawing,oldX,oldY,radArcMin,0)
##	oldX,oldY=shiftNextLine(oldX,oldY,radArcMin,40,4,3,radArcMin * 25,p0x)
##	oldX,oldY==drawPieceBottomForever(drawing,oldX,oldY,radArcMin,0)
##	oldX,oldY=shiftNextLine(oldX,oldY,radArcMin,40,4,3,radArcMin * 25,p0x)
##	oldX,oldY=drawPieceJointLoopWhite(drawing,oldX,oldY,radArcMin)
##	oldX,oldY=shiftNextLine(oldX,oldY,radArcMin,40,4,3,radArcMin * 25,p0x)
##	oldX,oldY=drawPieceRepeatLoopWhite(drawing,oldX,oldY,radArcMin)
##	oldX,oldY=shiftNextLine(oldX,oldY,radArcMin,40,4,3,radArcMin * 40,p0x)
##	oldX,oldY=drawPieceForeverLoopWhite(drawing,oldX,oldY,radArcMin)
	drawing.save()
